...
export default async (req, res) => {
  ...
  try{
    ...
    // 4. Get email for payment data, see if email linked with additional Stripe customer
     const prevCustomer = await stripe.customers.list({
      email: paymentData.email,
      limit: 1
    })
      const isExistingCustomer = prevCustomer.data.length > 0;
    // 5. If not exisiting customer, create them based on their email
    let newCustomer;
    ...
    const customer = (isExistingCustomer && prevCustomer.data[0]
      .id) || newCustomer.id;
    ...
    res.status(200).send("Checkout succesful")
